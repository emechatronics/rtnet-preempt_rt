...
#define SYS_socket_rtnet	436
#define SYS_bind_rtnet	437
#define SYS_recvmsg_rtnet	438
#define SYS_sendto_rtnet	439
#define SYS_recvfrom_rtnet	440
#define SYS_sendmsg_rtnet	441
...
#else
		//if (syscall(SYS_sendto_rtnet, sock, &tx_date, sizeof(tx_date), 0, (struct sockaddr *)&dest_addr, sizeof(struct sockaddr_in)) < 0) {
...
void *receiver(void *arg)
{
    ...
    pthread_setschedparam(pthread_self(), SCHED_FIFO, &param);
        ...
        if (ret <= 0) {
			...
			printf("recvmsg done\n");
#elif with_recvmsg
			printf("SYS_recvmsg_rtnet done: iov=%s ret=%d\n", (char*)iov.iov_base, ret);
#else
			recvbuf[ret] = 0;
